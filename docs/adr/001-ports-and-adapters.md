...
`DadEngine` holds the entire product: tap toggles, mode selection, emergency
...
`DadStore.shared`, `Shielder`, `DeviceActivityCenter()` and `Date()`. Every one
...
Invert the dependencies. `DadEngine` moves into `Dad/Shared/Core` and depends
...
| `DadPersisting` | the App Group `UserDefaults` | `UserDefaultsStore` |

`DadEngine` becomes a struct holding those four, constructed once at a
composition root (`DadEngine.live`) that only the iOS targets compile.

### Keeping FamilyControls out of Core

`DadMode` held a `FamilyActivitySelection`, which is a FamilyControls type, so
`DadMode` could not move to Core with it. But Core does not care *which* apps a
mode blocks — only whether it blocks anything, and how to describe it. Only the
shield adapter needs the real tokens.

So `DadMode.blocked` is a `BlockedSelection`: an opaque `Data` payload plus
...
`DadMode+FamilyControls` keeps that conversion in one place, both directions
adjacent.
