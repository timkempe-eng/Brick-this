...
1. **Request the Family Controls entitlement.** Development access is instant,
...
2. **Generate the project.** `brew install xcodegen && xcodegen generate`
3. **Set your Team ID** in `project.yml`, then `xcodegen generate` again.
4. **Add the Family Controls capability** to all four targets in Xcode, and
   **App Groups** (`group.app.tim.shared`) to all four. A mismatch here doesn't
   fail loudly — the shield just shows the wrong mode name — so check all four.
5. **Build to your iPhone.** Screen Time and NFC both no-op in the Simulator,
...
5. **Set up the Shortcuts automation** so the tap works with Tim closed:
...
6. **Stick the tag somewhere inconvenient.** This is the actual product. A
