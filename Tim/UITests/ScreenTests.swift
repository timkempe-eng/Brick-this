...
final class ScreenTests: XCTestCase {
    ...
    /// The Mode editor must accept a change at all.
    ///
    /// "Strict" is the simplest control on the screen, and for a while it did
    /// not work either — which is what proved the fault was the editor's own
    /// state rather than the schedule row, after four runs of reproducing the
    /// schedule failure without separating those two. Taps arrived and Cancel
    /// worked; only the editor's `@State` failed to survive. It holds a
    /// binding now instead of a copy.
    func testTheEditorAcceptsASimpleToggle() {
        ...
        XCTAssertEqual(strict.value as? String, "1",
                       """
                       The editor accepted no change at all.
