...
/// What the sessions add up to. All the arithmetic lives in `TimStats`, which
/// is covered by `swift test`; this file only lays it out.
struct StatsView: View {
    @EnvironmentObject private var model: TimModel
    @Environment(\.dismiss) private var dismiss

    private var stats: TimStats { model.stats }
                ...
                if stats.sessionCount == 0 {
                    Section {
                        ContentUnavailableView(
                            ...
                            description: Text("Tim your phone once and this fills in.")
                        )
                    }
                } else {
                    Section {
                        HStack(spacing: 12) {
                            Tile(value: "\(stats.currentStreak)",
                                 ...
                                 label: "Streak")
                            Tile(value: stats.timeThisWeek.timDurationText,
                                 label: "This week")
                        }
                    ...
                    Section {
                        ...
                        LabeledContent("Total time", value: stats.totalTime.timDurationText)
                        LabeledContent("Longest session", value: stats.longestSession.timDurationText)
                    ...
                    Section {
                        ...
                        LabeledContent(Vocab.emergencyUnTim, value: "\(stats.emergencyBails)")
    ...
    var body: some View {
        HStack(alignment: .bottom, spacing: 6) {
            ForEach(days) { day in
                ...
                .accessibilityValue(day.total > 0 ? day.total.timDurationText : "nothing")
            }
        }
    }
}
