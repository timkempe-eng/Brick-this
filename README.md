...
```bash
swift test                      # 145 tests, seconds, no Mac
...
session can begin or end. The engine depends only on protocols, which is what
lets the whole state machine be tested without a device —
[ADR 001](docs/adr/001-ports-and-adapters.md). The family layer will arrive the
same way: a role and a grant are new ports, not new framework imports in Core.
...
`swift test` passes — 145 tests covering the whole engine state machine,
recurring schedules, the override allowance, and the session/streak maths. The
suite is mutation-checked: deliberately breaking the tag guard, the allowance,
the empty-Mode guard, the history bound, the scheduler floor or the reconcile
clear each turns it red.

The app and all four extensions compile cleanly against the iOS SDK on a GitHub
macOS runner, on every push. Nothing has run on a device yet.
