...
final class TimModeTests: XCTestCase {

    func testBlocksAnythingFollowsTheCounts() {
        XCTAssertFalse(TimMode(name: "x", symbol: "c").blocksAnything)
        XCTAssertTrue(TimMode(name: "x", symbol: "c",
                              blocked: BlockedSelection(appCount: 1)).blocksAnything)
    }

    func testStrictIsCalledOutInTheSummary() {
        let mode = TimMode(name: "Sleep", symbol: "moon",
    ...
    func testModeSurvivesCoding() throws {
        let original = TimMode(name: "Deep Work", symbol: "brain",
                               blocked: BlockedSelection(payload: Data([1]), appCount: 4),
                               isStrict: true, autoUnTimAfter: 3600)
        let restored = try JSONDecoder().decode(TimMode.self, from: JSONEncoder().encode(original))
        XCTAssertEqual(restored, original)
    }
}
